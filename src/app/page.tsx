...
interface Translation {
  title: string; subtitle: string; invoke: string; records: string; oracle: string;
...
const TRANSLATIONS: Record<Lang, Translation> = {
  en: {
    title: "K-number", subtitle: "The Mystic Oracle", invoke: "Invoke", records: "Records",
    oracle: "Oracle", login: "Login", logout: "Logout", generateLuck: "GENERATE LUCK",
    invoking: "INVOKING...", selectMode: "Select Destiny Mode", numCount: "Number Count",
    range: "Max Range", connected: "Destiny Connected", noRecords: "The scrolls are empty.",
    resetProfile: "Reset My Destiny", awakenDestiny: "Awaken Your Destiny",
    oracleGuide: "The Oracle requires your birth alignment", nameLabel: "Name",
    dateLabel: "Birth Date", startOracle: "ACTIVATE ORACLE", luckyArea: "Fortune Zone",
    luckIndex: "Luck Index", shareTitle: "Destiny Card", close: "Close",
    elementAnalysis: "Cosmic Element Balance",
  },
  ko: {
    title: "K-포춘", subtitle: "신비로운 운세 상담소", invoke: "번호 생성", records: "행운 기록",
    oracle: "운세 상담", login: "로그인", logout: "로그아웃", generateLuck: "행운 불러오기",
    invoking: "기운 모으는 중...", selectMode: "운명의 모드 선택", numCount: "숫자 개수",
    range: "번호 범위", connected: "운명이 연결됨", noRecords: "아직 기록이 없습니다.",
    resetProfile: "프로필 초기화", awakenDestiny: "당신의 운명을 깨우세요",
    oracleGuide: "오행 분석을 위해 사주 정보를 입력해 주세요", nameLabel: "이름",
    dateLabel: "생년월일", startOracle: "운세 분석 시작", luckyArea: "행운의 영역",
    luckIndex: "오늘의 행운", shareTitle: "운명의 카드", close: "닫기",
    elementAnalysis: "오행 기운 밸런스",
  }
};
...
const ELEMENTS: ElementInfo[] = [
  { 
    ...
    messages: [
      { en: "Fresh energy flows like a young tree. Aim for low numbers today.", ko: "새싹 같은 기운이 솟아납니다. 낮은 번호대에 행운이 깃들어 있습니다." },
      { en: "Growth is imminent. Spread your luck across the board.", ko: "성장의 시기입니다. 번호를 넓게 배치하여 기운을 확산시키세요." }
    ]
  },
  { 
    name: "Fire", symbol: "火", color: "text-red-500", bg: "bg-red-500/10", desc: "열정과 확산", range: [11, 20],
    messages: [
      { en: "A burning passion will light your way. Hot numbers are calling.", ko: "뜨거운 열정이 운명을 밝힙니다. 최근 자주 나온 번호에 주목하세요." },
      { en: "The sun is at its peak. High energy resides in the mid-range.", ko: "태양이 정점에 올랐습니다. 중간 번호대에 강력한 에너지가 모여있습니다." }
    ]
  },
  { 
    name: "Earth", symbol: "土", color: "text-yellow-600", bg: "bg-yellow-600/10", desc: "안정과 균형", range: [21, 30],
    messages: [
      { en: "Find stability in balance. Earthy tones bring steady luck.", ko: "균형 속에서 안정을 찾으세요. 차분한 번호 선택이 승리를 가져옵니다." },
      { en: "The foundation is strong. Focus on the core numbers today.", ko: "기초가 튼튼한 날입니다. 중심이 되는 번호들을 놓치지 마세요." }
    ]
  },
  { 
    name: "Metal", symbol: "金", color: "text-gray-400", bg: "bg-gray-400/10", desc: "결단과 결실", range: [31, 40],
    messages: [
      { en: "Sharp intuition will lead to harvest. Golden opportunities await.", ko: "날카로운 직관이 결실을 맺습니다. 금빛 기운의 번호를 선택하세요." },
      { en: "Fortitude is your strength today. Solid numbers provide the key.", ko: "강인함이 무기입니다. 단단한 기운의 높은 번호대를 공략하세요." }
    ]
  },
  { 
    name: "Water", symbol: "水", color: "text-blue-500", bg: "bg-blue-500/10", desc: "지혜와 유연함", range: [41, 50],
    messages: [
      { en: "Wisdom flows like a river. Let your luck drift to the deep end.", ko: "지혜가 강물처럼 흐릅니다. 깊이 있는 높은 번호들이 길합니다." },
      { en: "Adaptability is key. Fluid choices will bring the best results.", ko: "유연함이 핵심입니다. 물 흐르듯 자연스러운 번호 조합을 믿으세요." }
    ]
  },
];
      ...
      <header className="px-6 py-8 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white/95 dark:bg-black/95 backdrop-blur-md z-10">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setLang(lang === "ko" ? "en" : "ko"); triggerHaptic(); }} className="px-2 py-0.5 bg-gray-100 dark:bg-neutral-800 rounded-md text-[10px] font-black text-gray-500 uppercase tracking-tighter transition-all active:scale-90">
              {lang === "ko" ? "EN" : "KO"}
            </button>
          </div>
        ...
        <div className="flex flex-col items-end">
          {user ? (
            ...
            <button onClick={() => { supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } }); triggerHaptic(); }} className="px-5 py-2.5 bg-blue-600 text-white text-[10px] font-black rounded-full uppercase tracking-tighter shadow-lg shadow-blue-500/20 active:scale-95 transition-all">{t.login}</button>
          )}
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 pb-40">
        {activeTab === "generate" && (
          <div className="flex flex-col gap-8 animate-in fade-in duration-300">
            ...
            <section className="bg-neutral-950 rounded-[3.5rem] p-10 shadow-2xl relative overflow-hidden border border-blue-900/20 text-center min-h-[340px] flex flex-col items-center justify-center">
              ...
              {numbers.length === 0 ? (
                ...
                <div className="flex flex-col items-center gap-12 w-full relative z-10">
                  <div className="flex justify-center gap-4 flex-wrap max-w-xs">
                    {numbers.map((num, idx) => (
                      <div key={idx} className={`w-14 h-14 flex items-center justify-center rounded-full text-xl font-black shadow-2xl transform transition-all duration-500 ${idx < visibleCount ? "scale-100 opacity-100 rotate-0" : "scale-0 opacity-0 rotate-180"} ${getBallColor(num, selectedLotto.id === "custom" ? customMax : selectedLotto.max)}`} style={{ boxShadow: "inset -4px -4px 10px rgba(0,0,0,0.2), 0 10px 20px rgba(0,0,0,0.3)" }}>{num}</div>
                    ))}
                  </div>
          ...
          <div className="flex flex-col gap-6 animate-in slide-in-from-right-4 duration-300">
            ...
            {!user ? (
              ...
              <div className="grid grid-cols-1 gap-5">
                {history.map((item) => (
                  <div key={item.id} className="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 p-7 rounded-[2.5rem] shadow-sm relative group hover:shadow-md transition-all">
                    <div className="flex justify-between items-center mb-5">
                      <div className="flex items-center gap-3"><span className="text-[10px] font-black text-white uppercase bg-blue-600 px-4 py-1.5 rounded-full">{item.lottery_name}</span><span className="text-xs text-gray-400 font-bold tracking-tight">{item.timestamp}</span></div>
                      <button onClick={() => { triggerHaptic(); if(confirm("Delete?")) supabase.from("lotto_history").delete().match({ id: item.id }).then(() => fetchHistory()); }} className="text-gray-300 hover:text-red-500 transition-all p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18m-2 0v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6m3 0V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></button>
                    </div>
              ...
              <div className="space-y-10">
                ...
                <div className={`p-10 rounded-[4.5rem] shadow-2xl ${luckyElement.bg} border border-white/5 relative overflow-hidden transition-all duration-1000`}>
                  ...
                  <div className="mt-12 p-8 bg-white/40 dark:bg-black/30 rounded-[2.5rem] backdrop-blur-md relative z-10 border border-white/10">
                    <p className="text-lg font-bold leading-relaxed text-gray-800 dark:text-gray-100">{oracleMessage}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest px-2">{t.elementAnalysis}</h3>
                  <div className="bg-white dark:bg-neutral-900 rounded-[3rem] p-8 space-y-6 border border-gray-100 dark:border-neutral-800 shadow-sm">
                    {ELEMENTS.map((el) => {
                      const isActive = el.name === luckyElement.name;
                      return (
                        <div key={el.name} className="space-y-2">
                          <div className="flex justify-between items-end"><span className={`text-[10px] font-black uppercase tracking-tighter ${el.color}`}>{el.name} {el.symbol}</span><span className="text-[10px] font-black text-gray-400">{isActive ? "Peak" : "Steady"}</span></div>
                          <div className="h-2 w-full bg-gray-100 dark:bg-neutral-800 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${isActive ? "bg-blue-600 w-full" : "bg-gray-300 dark:bg-gray-700 w-1/3"}`}></div></div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="bg-neutral-950 text-white p-10 rounded-[3rem] shadow-2xl border border-blue-900/30 flex items-start gap-6 relative overflow-hidden group">
                  ...
                  <div className="space-y-3 pt-1 relative z-10"><span className="text-[10px] font-black uppercase text-blue-400 tracking-[0.2em]">Oracle's Strategy</span><p className="text-xl font-bold leading-tight italic text-gray-100">"{luckyElement.symbol}의 기운이 충만합니다. 번호 생성 시 {luckyElement.range[0]}~{luckyElement.range[1]}번 영역에 행운의 씨앗을 심어두었습니다."</p></div>
                </div>
          ...
          <div className="relative w-full max-w-sm bg-gradient-to-b from-neutral-900 to-black rounded-[3.5rem] shadow-[0_0_100px_rgba(37,99,235,0.3)] overflow-hidden border border-white/10 flex flex-col p-10 gap-8 animate-in zoom-in duration-500">
            ...
            <div className={`p-8 rounded-[2.5rem] ${luckyElement.bg} border border-white/5 relative overflow-hidden text-center`}>
              ...
              <div className="relative z-10 space-y-6">
                <p className="text-xs font-black text-gray-400 uppercase tracking-widest">{userProfile?.name || "Seeker"}'s Luck</p>
                <div className="flex justify-center gap-3 flex-wrap">{numbers.map((num, i) => ( <div key={i} className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-black ${getBallColor(num, 70)} shadow-lg`}>{num}</div> ))}</div>
