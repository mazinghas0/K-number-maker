...
const ELEMENTS: ElementInfo[] = [
  { name: "Wood", symbol: "木", color: "text-green-500", bg: "bg-green-500/10", desc: "성장과 활력", range: [1, 10], messages: [{ en: "Energy flows like a tree.", ko: "나무의 기운이 흐릅니다.", ja: "木のエネルギー.", es: "Energía de árbol." }, { en: "Spread branches.", ko: "가지를 뻗으세요.", ja: "枝を広げましょう.", es: "Extiende ramas." }] },
  { name: "Fire", symbol: "火", color: "text-red-500", bg: "bg-red-500/10", desc: "열정과 확산", range: [11, 20], messages: [{ en: "Passion lights way.", ko: "열정이 길을 밝힙니다.", ja: "情熱が道を照らす.", es: "La pasión ilumina." }, { en: "Sun peaks.", ko: "태양이 정점입니다.", ja: "太陽が頂点.", es: "El sol cenit." }] },
  { name: "Earth", symbol: "土", color: "text-yellow-600", bg: "bg-yellow-600/10", desc: "안정과 균형", range: [21, 30], messages: [{ en: "Stability luck.", ko: "안정이 행운을 부릅니다.", ja: "安定が幸運を呼ぶ.", es: "Estabilidad suerte." }, { en: "Firm foundation.", ko: "기초가 탄탄한 날.", ja: "土台がしっかり.", es: "Cimientos firmos." }] },
  { name: "Metal", symbol: "金", color: "text-gray-400", bg: "bg-gray-400/10", desc: "결단과 결실", range: [31, 40], messages: [{ en: "Intuition gold.", ko: "직관이 결실을 맺습니다.", ja: "直感が実を結ぶ.", es: "Intuición oro." }, { en: "Sharpness strength.", ko: "날카로움이 힘입니다.", ja: "鋭さが力.", es: "Agudeza fuerza." }] },
  { name: "Water", symbol: "水", color: "text-blue-500", bg: "bg-blue-500/10", desc: "지혜와 유연함", range: [41, 50], messages: [{ en: "Wisdom flows.", ko: "지혜가 흐릅니다.", ja: "知恵が流れる.", es: "Sabiduría fluye." }, { en: "Adaptability key.", ko: "유연함이 핵심.", ja: "柔軟さが鍵.", es: "Adaptabilidad clave." }] },
];
    ...
    <>
      <header className="px-6 py-8 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white/95 dark:bg-black/95 backdrop-blur-md z-10">
        <div className="flex flex-col">
          <div className="flex items-center gap-2"><h1 className="text-3xl font-black tracking-tighter italic leading-none">{t.title}</h1><div className="flex gap-1">{(["ko", "en", "ja", "es"] as Lang[]).map((l) => ( <button key={l} onClick={() => { setLang(l); triggerHaptic(); }} className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase transition-all ${lang === l ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-neutral-800 text-gray-400"}`}>{l}</button> ))}</div></div>
          <p className="text-[9px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-2">{t.subtitle}</p>
        </div>
        <div className="flex flex-col items-end">
          {user ? (
            <div className="flex items-center gap-3"><div className="flex flex-col items-end"><span className="text-sm font-black text-blue-500 uppercase">{userProfile?.name || "Seeker"}</span><span className="text-[9px] text-gray-400 tracking-tighter">{t.connected}</span></div><button onClick={() => { supabase.auth.signOut(); triggerHaptic(); }} className="w-9 h-9 rounded-2xl bg-gray-100 dark:bg-neutral-800 flex items-center justify-center text-sm shadow-sm">🚪</button></div>
          ) : (
          ...
          <div className="flex flex-col gap-8 animate-in slide-in-from-right-4 duration-300 pb-10">
            {!userProfile ? (
              ...
              <div className="space-y-10">
                ...
                <div className={`p-10 rounded-[4.5rem] shadow-2xl ${luckyElement.bg} border border-white/5 relative overflow-hidden transition-all duration-1000`}><div className="flex justify-between items-start relative z-10"><div className="space-y-2"><span className={`text-[10px] font-black uppercase tracking-[0.3em] ${luckyElement.color}`}>Primary Element</span><h3 className="text-6xl font-black tracking-tighter">{luckyElement.name}</h3><p className="text-sm font-bold opacity-60 italic">{luckyElement.desc}</p></div><span className={`text-[12rem] font-serif opacity-10 ${luckyElement.color} absolute -right-12 -top-12`}>{luckyElement.symbol}</span></div><div className="mt-12 p-8 bg-white/40 dark:bg-black/30 rounded-[2.5rem] backdrop-blur-md relative z-10 border border-white/10"><p className="text-xl font-black leading-relaxed text-gray-800 dark:text-gray-100">{oracleMessage}</p></div></div>
                <div className="space-y-6"><h3 className="text-lg font-black text-gray-400 uppercase tracking-widest px-2">{t.elementAnalysis}</h3><div className="bg-white dark:bg-neutral-900 rounded-[3rem] p-10 space-y-10 border border-gray-100 dark:border-neutral-800 shadow-sm">{ELEMENTS.map((el) => { const isActive = el.name === luckyElement.name; return ( <div key={el.name} className="space-y-4"><div className="flex justify-between items-end"><span className={`text-xl font-black uppercase tracking-tighter ${el.color}`}>{el.name} {el.symbol}</span><span className="text-xs font-black text-gray-400">{isActive ? "PEAK ENERGY" : "STEADY"}</span></div><div className="h-4 w-full bg-gray-100 dark:bg-neutral-800 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${isActive ? "bg-blue-600 w-full" : "bg-gray-300 dark:bg-gray-700 w-1/3"}`}></div></div></div> ); })}</div></div>
                <div className="bg-neutral-950 text-white p-10 rounded-[3rem] shadow-2xl border border-blue-900/30 flex items-start gap-6 relative overflow-hidden group"><div className="absolute top-0 right-0 w-24 h-24 bg-blue-600/10 rounded-full blur-3xl"></div><div className="w-16 h-16 bg-blue-600 rounded-[1.5rem] flex items-center justify-center text-4xl shadow-lg shrink-0 animate-bounce">🤖</div><div className="space-y-3 pt-1 relative z-10"><span className="text-[10px] font-black uppercase text-blue-400 tracking-[0.2em]">Oracle's Strategy</span><p className="text-xl font-bold leading-tight italic text-gray-100">"{luckyElement.symbol}의 기운이 충만합니다. 번호 생성 시 {luckyElement.range[0]}~{luckyElement.range[1]}번 영역에 행운의 씨앗을 심어두었습니다."</p></div></div>
                <div className="grid grid-cols-2 gap-4"><div className="bg-gray-50 dark:bg-neutral-900 p-8 rounded-[2.5rem] text-center space-y-1 border border-gray-100 dark:border-neutral-800"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest opacity-60">{t.luckIndex}</span><p className="text-4xl font-black text-blue-600">{luckScore}%</p></div><div className="bg-gray-50 dark:bg-neutral-900 p-8 rounded-[2.5rem] text-center space-y-1 border border-gray-100 dark:border-neutral-800"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest opacity-60">{t.luckyArea}</span><p className="text-4xl font-black text-blue-600">{luckyElement.range[0]}~{luckyElement.range[1]}</p></div></div>
