...
export default function Home() {
  ...
  const shareLuck = async () => {
    ...
    if (navigator.share) {
      ...
      alert(lang === "ko" ? "행운의 메시지가 복사되었습니다!" : "Luck message copied!");
    }
  };
      ...
      <header className="px-6 py-8 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white/95 dark:bg-black/95 backdrop-blur-md z-10">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={() => { setLang(lang === "ko" ? "en" : "ko"); triggerHaptic(); }}
              className="px-2 py-0.5 bg-gray-100 dark:bg-neutral-800 rounded-md text-[10px] font-black text-gray-500 uppercase tracking-tighter"
            >
              {lang === "ko" ? "EN" : "KO"}
            </button>
          </div>
        ...
        <div className="flex flex-col items-end">
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { supabase.auth.signOut(); triggerHaptic(); }} className="w-9 h-9 rounded-2xl bg-gray-100 dark:bg-neutral-800 flex items-center justify-center text-sm">🚪</button>
            </div>
          ) : (
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 pb-40">
        {activeTab === "generate" && (
          <div className="flex flex-col gap-8 animate-in fade-in duration-300">
            ...
            {selectedLotto.id === "custom" && (
              <section className="bg-gray-50 dark:bg-neutral-900/50 p-8 rounded-[2.5rem] space-y-8 animate-in slide-in-from-top-4 border border-gray-100 dark:border-neutral-800">
                <div className="space-y-4">
                  <div className="flex justify-between items-center"><span className="text-sm font-black text-gray-400 uppercase">{t.numCount}</span><span className="text-xl font-black text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-4 py-1 rounded-full">{customCount}</span></div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center"><span className="text-sm font-black text-gray-400 uppercase">{t.range}</span><span className="text-xl font-black text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-4 py-1 rounded-full">1 ~ {customMax}</span></div>
            ...
            <section className="bg-neutral-950 rounded-[3.5rem] p-10 shadow-2xl relative overflow-hidden border border-blue-900/20 text-center min-h-[320px] flex flex-col items-center justify-center">
              ...
              {numbers.length === 0 ? (
                ...
                <div className="flex flex-col items-center gap-10 w-full animate-in zoom-in duration-500 relative z-10">
                  ...
                  <button onClick={shareLuck} className="px-8 py-3 bg-white/5 border border-white/10 rounded-full hover:bg-white/10 transition-all flex items-center gap-2">
          ...
          <div className="flex flex-col gap-6 animate-in slide-in-from-right-4 duration-300">
            ...
            {!user ? (
              <div className="text-center py-20 bg-gray-50 dark:bg-neutral-900 rounded-[3.5rem] p-10 space-y-8 border-2 border-dashed border-gray-100 dark:border-neutral-800">
                ...
                <p className="text-base font-bold text-gray-500 leading-relaxed">로그인하시면 클라우드에 영구적으로<br/>기록을 보관할 수 있습니다.</p>
                <button onClick={() => { supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } }); triggerHaptic(); }} className="w-full py-5 bg-blue-600 text-white font-black rounded-3xl uppercase text-sm tracking-widest shadow-xl shadow-blue-500/20 active:scale-95 transition-all">Sign in with Google</button>
              </div>
            ) : history.length === 0 ? (
                ...
                {history.map((item) => (
                  <div key={item.id} className="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 p-7 rounded-[2.5rem] shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    <div className="flex justify-between items-center mb-5">
                      ...
                      <button onClick={() => { triggerHaptic(); if(confirm("Delete record?")) supabase.from("lotto_history").delete().match({ id: item.id }).then(() => fetchHistory()); }} className="text-gray-300 hover:text-red-500 transition-all p-2 rounded-full">
            ...
            {!userProfile ? (
              <section className="bg-white dark:bg-neutral-900 p-10 rounded-[3.5rem] shadow-2xl border border-gray-100 dark:border-neutral-800">
                <div className="text-center space-y-4 mb-10">
                  ...
                  <h2 className="text-3xl font-black tracking-tighter uppercase">{t.awakenDestiny}</h2>
                  <p className="text-xs text-gray-400 font-black uppercase tracking-[0.2em]">{t.oracleGuide}</p>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); localStorage.setItem("k-fortune-profile", JSON.stringify(tempProfile)); setUserProfile(tempProfile); triggerHaptic(); }} className="space-y-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-400 ml-2 tracking-widest uppercase">{t.nameLabel}</label>
                    <input type="text" placeholder="Fortune Seeker" className="w-full px-8 py-5 rounded-3xl bg-gray-50 dark:bg-neutral-800 border-none font-black text-lg focus:ring-4 focus:ring-blue-500/20 transition-all" value={tempProfile.name} onChange={(e) => setTempProfile({...tempProfile, name: e.target.value})} required />
                  </div>
                  <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-400 ml-2 tracking-widest uppercase">{t.dateLabel}</label>
                      <input type="date" className="w-full px-8 py-5 rounded-3xl bg-gray-50 dark:bg-neutral-800 border-none font-black text-lg focus:ring-4 focus:ring-blue-500/20" value={tempProfile.birthDate} onChange={(e) => setTempProfile({...tempProfile, birthDate: e.target.value})} required />
                    </div>
                  </div>
                  <button type="submit" className="w-full bg-blue-600 text-white font-black py-6 rounded-[2.5rem] shadow-2xl shadow-blue-600/30 hover:bg-blue-700 active:scale-95 transition-all text-xl uppercase tracking-widest">{t.startOracle}</button>
                </form>
              </section>
            ) : (
              <div className="space-y-10">
                <div className="text-center space-y-2">
                  <h2 className="text-4xl font-black italic tracking-tighter">{t.oracle}</h2>
                ...
                <div className={`p-10 rounded-[4.5rem] shadow-2xl ${luckyElement.bg} border border-white/5 relative overflow-hidden transition-all duration-1000`}>
                  ...
                  <div className="mt-12 p-8 bg-white/40 dark:bg-black/30 rounded-[2.5rem] backdrop-blur-md relative z-10 border border-white/10">
                    <p className="text-lg font-bold leading-relaxed text-gray-800 dark:text-gray-100">
                      {lang === "ko" ? `오늘 당신의 수호 기운은 ${luckyElement.name}입니다. 생성되는 번호에 이 에너지가 담겨 승리의 진동을 높일 것입니다.` : `Your dominant energy today is ${luckyElement.name}. The numbers generated carry this vibration to enhance your victory.`}
                    </p>
                  </div>
                </div>
